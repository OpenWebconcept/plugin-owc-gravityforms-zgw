...
class FormSettings
{
	...
	public function add_form_settings(array $fields, array $form ): array
	{
		$fields['owc-gravityforms-aanvraagtype'] = array(
			'title'       => esc_html__( 'ZGW component', 'owc-gravityforms-zgw' ),
			'description' => esc_html__( 'Selecteer een ZGW component waarvan een object aangemaakt moet worden bij een formulier inzending.', 'owc-gravityforms-zgw' ),
			'fields'      => array(
				array(
					'name'          => "{$this->prefix}-form-setting-zgw-component",
					'default_value' => "{$this->prefix}-form-setting-zgw-component-none",
					'type'          => 'select',
					'label'         => esc_html__( 'Selecteer component', 'owc-gravityforms-zgw' ),
					'choices'       => $this->handle_aanvraag_type_choices(),
				),
			),
		);

		$fields['owc-gravityforms-verzoek'] = array(
			'title'       => esc_html__( 'ZGW Verzoeken', 'owc-gravityforms-zgw' ),
			'description' => esc_html__( 'Instellingen voor het aanmaken van een Verzoek object bij een formulier inzending.', 'owc-gravityforms-zgw' ),
			'dependency'  => array(
				'live'   => true,
				'fields' => array(
					array(
						'field'  => "{$this->prefix}-form-setting-zgw-component",
			...
			'fields'      => array(
				array(
					'name'    => "{$this->prefix}-form-setting-verzoeken-information-object-type",
					'type'    => 'select',
				...
				array(
					'name'    => "{$this->prefix}-form-setting-verzoeken-object-type",
					'type'    => 'select',
					'label'   => esc_html__( 'Object type', 'owc-gravityforms-zgw' ),
					'choices' => array_map(
						function ($type ) {
							return array(
								'label' => $type['name'],
						...
						( new \OWCGravityFormsZGW\Verzoeken\Client() )->object_types()['results'] ?? array()
					),
				),
				array(
					'name'    => "{$this->prefix}-form-setting-verzoeken-overwrite-bsn",
					'type'    => 'text',
				...
				array(
					'name'    => "{$this->prefix}-form-setting-verzoeken-overwrite-kvk",
					'type'    => 'text',
		...
		$fields['owc-gravityforms-zaaksysteem'] = array(
			'title'       => esc_html__( 'ZGW Zaken', 'owc-gravityforms-zgw' ),
