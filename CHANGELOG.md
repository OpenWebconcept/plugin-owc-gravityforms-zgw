...
-   Fix: log unreadable or empty submission PDF and add upload tests
-   Fix: log empty response body when retrieving an uploaded file (#40) (@Scottielee33)
-   Fix: do not create an informatieobject when file content is empty (#40) (@Scottielee33)

## [v1.17.3] - 2026-09-22

-   Added: button to exclude fields from PDF output
...
-   Change: standardize filter hook names under the `owcgfzgw::` namespace (`owc_gf_zgw_form_settings_cache_capability` →
    `owcgfzgw::form_settings/cache_capability`, `owc_zgw_transaction_cleanup_days` →
    `owcgfzgw::transaction/cleanup_days`); breaking change for integrations relying on the old filter names
